...
// THE DEFENSIVE TOKEN VALIDATOR
function isValidToken(token: string): boolean {
  // Defensively strip "Bearer " and any extra spaces if the middleware passed them by accident
  ...
  console.log(`Total Active OAuth Tokens in Memory: ${accessTokens.size}`);

  // Accept our issued OAuth tokens
  const entry = accessTokens.get(cleanToken);
  if (entry) {
    if (entry.expires_at > Date.now()) {
      console.log(`✅ Result: Token valid and active! Letting Claude in.`);
      return true;
    } else {
      console.log(`❌ Result: Token expired.`);
      return false;
    }
  }
  ...
  if (STATIC_TOKEN && cleanToken === STATIC_TOKEN) {
    console.log(`✅ Result: Matched STATIC_TOKEN`);
    return true;
  }

  console.log(`❌ Result: Token NOT found in memory. Rejecting request.`);
  return false;
}
...
// FORCE EXPRESS TO PARSE STANDARD OAUTH FORM DATA
...
// 6. Token Endpoint (WITH HEAVY LOGGING)
oauthRouter.post('/oauth/token', (req: Request, res: Response) => {
  ...
  if (grant_type !== 'authorization_code') {
    console.error("Token Error: unsupported_grant_type ->", grant_type);
  ...
  const authCode = authCodes.get(code);
  if (!authCode) {
    console.error("Token Error: Code not found in memory. (Server may have restarted)");
    res.status(400).json({ error: 'invalid_grant', error_description: 'Code not found' });
    return;
  }

  if (!authCode.approved) {
    console.error("Token Error: Code exists but was never marked approved.");
    res.status(400).json({ error: 'invalid_grant', error_description: 'Code not approved' });
    return;
  }

  if (authCode.expires_at < Date.now()) {
    console.error("Token Error: Code expired.");
  ...
  if (authCode.client_id !== client_id) {
    console.error(`Token Error: client_id mismatch. Expected [${authCode.client_id}] got [${client_id}]`);
    res.status(400).json({ error: 'invalid_client' });
    return;
  }

  if (authCode.redirect_uri !== redirect_uri) {
    console.error(`Token Error: redirect_uri mismatch. Expected [${authCode.redirect_uri}] got [${redirect_uri}]`);
    res.status(400).json({ error: 'invalid_grant', error_description: 'redirect_uri mismatch' });
    return;
  }

  if (!verifyPKCE(code_verifier, authCode.code_challenge, authCode.code_challenge_method)) {
    console.error("Token Error: PKCE verification failed.");
    res.status(400).json({ error: 'invalid_grant', error_description: 'PKCE verification failed' });
    return;
  }

  console.log("✅ Token exchange successful! Issuing access token to Claude.");
